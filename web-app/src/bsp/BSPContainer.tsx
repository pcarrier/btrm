...
export function BSPContainer({
  ...
  // Track what we last told Workspace, to distinguish external changes from our own.
  const pushedRef = useRef(focusedSessionId);
  ...
  // Sync from external focusedSessionId (e.g. cmd-k selecting a terminal).
  if (focusedSessionId !== pushedRef.current && focusedSessionId != null) {
    const paneId = paneIds.find((id) => layoutState.assignments[id] === focusedSessionId);
    if (paneId && paneId !== focusedPaneId) setFocusedPaneId(paneId);
    pushedRef.current = focusedSessionId;
  }

  // Push our derived session up to Workspace.
  useEffect(() => {
    if (focusedPaneSessionId !== focusedSessionId) {
      pushedRef.current = focusedPaneSessionId;
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderBottom: index === activeTab ? `2px solid ${theme.accent}` : "2px solid transparent",
                }}
              >
        ...
        <Fragment key={index}>
          {index > 0 && (
            <ResizeHandle
              ...
              onDrag={(fraction) => onResizeRef.current(nodeRef.current, index - 1, index, fraction)}
            />
          )}
          <div
            style={{
...
function LeafPane({
  ...
  const termRef = useCallback((handle: BlitTerminalHandle | null) => {
  ...
  return (
    <div
      style={{
        ...
        position: "relative",
        border: multiPane
          ? isFocused ? `2px solid ${theme.accent}` : `2px solid transparent`
          : "none",
      }}
      ...
      {sessionId ? (
        <>
          <BlitTerminal
            ...
            showCursor={!multiPane || isFocused}
          />
