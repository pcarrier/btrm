...
function sendFd(channel: number, clientFd: number) {
  ...
  if (Number(libc.symbols.sendmsg(channel, ptr(new Uint8Array(msg.buffer)), 0)) < 0)
...
function readExact(fd: number, size: number): Uint8Array {
  ...
  let offset = 0;
  while (offset < size) {
    const n = Number(libc.symbols.read(fd, ptr(buf.subarray(offset)), BigInt(size - offset)));
...
function writeAll(fd: number, data: Uint8Array) {
  ...
  while (offset < data.length) {
    const n = Number(libc.symbols.write(fd, ptr(data.subarray(offset)), BigInt(data.length - offset)));
...
try {
  ...
  assert(hello[0] === S2C_HELLO, `expected HELLO (0x07), got 0x${hello[0].toString(16)}`);
  ...
  assert(list[0] === S2C_LIST, `expected LIST (0x03), got 0x${list[0].toString(16)}`);
  ...
  assert(ready[0] === S2C_READY, `expected READY (0x09), got 0x${ready[0].toString(16)}`);
  console.log("READY");
  ...
  assert(created[0] === S2C_CREATED, `expected CREATED (0x01), got 0x${created[0].toString(16)}`);
