...
describe("useBlitSessions — advanced scenarios", () => {
  ...
  it("LIST with same entries is idempotent", () => {
    ...
    expect(result.current.sessions.every((s) => s.state === "active")).toBe(
      true,
    );
  });
  ...
  it("empty LIST marks everything closed", () => {
    ...
    expect(result.current.sessions.every((s) => s.state === "closed")).toBe(
      true,
    );
  });
  ...
  it("handles high pty IDs (u16 range)", () => {
    ...
    expect(result.current.sessions.find((s) => s.ptyId === 65535)?.tag).toBe(
      "max",
    );
    expect(result.current.sessions.find((s) => s.ptyId === 256)?.tag).toBe(
      "mid",
    );
  });
  ...
  it("sessions reconcile on reconnect LIST", () => {
    ...
    expect(result.current.sessions.find((s) => s.ptyId === 1)?.state).toBe(
      "closed",
    );
    expect(result.current.sessions.find((s) => s.ptyId === 2)?.state).toBe(
      "active",
    );
    expect(result.current.sessions.find((s) => s.ptyId === 3)?.state).toBe(
      "active",
    );
    expect(result.current.sessions.find((s) => s.ptyId === 1)?.title).toBe(
      "vim",
    );
  });
