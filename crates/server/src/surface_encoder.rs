...
// SurfaceEncodeWorker
//
// Owns a SurfaceEncoder on a dedicated OS thread.  The async side sends
// EncodeJob values and polls for EncodeResult values — no blocking, no mutex
// held during H.264 encode.
// ---------------------------------------------------------------------------
...
pub struct SurfaceEncodeWorker {
    pub job_tx: std_mpsc::SyncSender<EncodeJob>,
    pub result_rx: std_mpsc::Receiver<EncodeResult>,
    pub current_dims: (u32, u32),
    _thread: std::thread::JoinHandle<()>,
}

impl SurfaceEncodeWorker {
    /// Spawn a new worker for `surface_id` at `(width, height)`.
    /// Returns `None` if the encoder cannot be created (e.g. odd dimensions).
    /// `notify` is called after each encoded frame so the delivery loop wakes
    /// up promptly to drain results and send frames to clients.
    pub fn new(
        surface_id: u16,
        width: u32,
        height: u32,
        ...
        let mut encoder = SurfaceEncoder::new(preference, width, height, vaapi_device, verbose)
            .map_err(|e| {
                if verbose {
                    eprintln!("[surface-encoder] failed to create encoder for surface {surface_id} at {width}x{height}: {e}");
                }
            })
            .ok()?;
        ...
        // Bounded at 1 so the async side can check without blocking: if the
        // previous job hasn't been consumed yet we just skip this frame.
        let (job_tx, job_rx) = std_mpsc::sync_channel::<EncodeJob>(1);
        let (result_tx, result_rx) = std_mpsc::sync_channel::<EncodeResult>(4);

        let thread = std::thread::spawn(move || {
            while let Ok(job) = job_rx.recv() {
                if job.want_keyframe {
                    encoder.request_keyframe();
                }
                if let Some((nal_data, is_keyframe)) = encoder.encode(&job.pixels) {
    ...
    /// Non-blocking send.  Returns false if the worker's input queue is full
    /// (previous frame still encoding) — caller should drop this frame.
    pub fn try_send(&self, job: EncodeJob) -> bool {
        self.job_tx.try_send(job).is_ok()
    }

    /// Drain all completed results without blocking.
    pub fn drain_results(&self) -> impl Iterator<Item = EncodeResult> + '_ {
